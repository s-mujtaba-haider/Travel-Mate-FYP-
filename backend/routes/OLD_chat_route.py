...
async def add_message(
    ...
    try:
        ...
        db.flush()
        try:
            response = await rag.answer_query(query=query, session_id=session_id, n_places=num_places)
        except SearchError:
            return HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Cant find any places')
        except RAGError:
            return HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Failed to initialize System')
        except Exception as e:
            return HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Some error occured on the server')
        ai_message = Message(
            session_id=session_id,
            role="assistant",
